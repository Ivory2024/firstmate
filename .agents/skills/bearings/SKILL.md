...
- The "unanswered questions" stat and table need no composer work: the board reads them directly off `captains_call` (every open Captain's Call item is, by definition, an unanswered question), with `key` as the id and `title` as the question text. This fleet's decision records carry no urgency concept, so the board's table has no urgency column - do not add one to the payload.
