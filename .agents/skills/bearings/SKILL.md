...
- When composing the unified `태스크 (M3)` table and task metric, request the snapshot with `--all-in-flight --all-queued --all-landed` so in-flight, queued, and landed inputs are complete; copy any actual remaining in-flight omission count to optional `underway_more`, and keep `charted_more` and `charted_warning_more` as the actual queued and warning omission counts.
...
- `fm-bearings-board.sh build` replaces payload metrics with read-only local sources: Claude 5-hour and 7-day quota percentages from `quota-axi --provider claude --json --no-credential-refresh`, cache-token usage and tool-result errors from the last 24 hours of `~/.claude/projects/*/*.jsonl`, and task counts from the complete `landed` and `underway` arrays. Quota cards are percentages because this subscription plan exposes no per-token dollar billing. Cache and tool-error cards describe the local Claude transcript activity in that 24-hour window. Context-read misses and auto-continue outcomes have no structured observable source, so those cards remain "no data"; never infer them from hook or API errors.
