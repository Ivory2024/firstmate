...
- `fm-bearings-board.sh build` replaces payload metrics with read-only local sources: Claude 5-hour and 7-day quota percentages from `quota-axi --provider claude --json --no-credential-refresh`, cache-token usage and tool-result errors from Claude JSONL transcripts under `~/.claude/projects` in the last 24 hours, and task counts from the complete `landed` and `underway` arrays. Quota cards are percentages because this subscription plan exposes no per-token dollar billing. Cache and tool-error cards describe local Claude transcript activity in that 24-hour window. Context-read misses and auto-continue outcomes have no structured observable source, so those cards remain "no data"; never infer them from hook or API errors.
