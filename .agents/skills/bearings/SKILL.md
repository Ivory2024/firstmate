...
- When composing the unified `태스크 (M3)` table, request the snapshot with `--all-in-flight --all-queued` so both task inputs are complete; copy any actual remaining in-flight omission count to optional `underway_more`, and keep `charted_more` and `charted_warning_more` as the actual queued and warning omission counts.
...
- A Charted Next row's optional `kind` separates work from alarms: omit it (or set `"queued"`) for real queued work, and set `"warning"` on every action-free fleet-integrity notice - the `(main-inventory)` gate, the `(return-catchup)` gate, an unavailable secondmate home, and an inventory-mismatch repair notice. In the unified task table, the board badges a warning row `needs repair` and excludes it from queued-work counts and dispatch selection.
- `charted_more` counts omitted queued rows only, while `charted_warning_more` counts omitted warning rows only; keep both counts separate whenever the board payload truncates Charted Next. `underway_more` counts omitted in-flight rows. Omit any count that the snapshot does not report.
- For each in-flight row, copy `in_flight.name` to `name` and preserve the snapshot id, state, and any actual blocker; in the unified task table the name is the title and the id remains the ID.
...
- Every Charted Next row copies the snapshot gate's durable filed date into `filed`, and the board orders charted rows by it, newest filed first after the in-flight rows.
...
- An Underway row MAY carry `blocker`: real structured text, cross-referenced from that same task id's `decisions_open` summary or `gates` reason in the snapshot, when one actually exists for that task. Omit it when the task has none; never invent a placeholder. A Charted Next row already has its blocker text in the existing `reason` field - the board renders that as a labeled blocker column, so compose it the same way you always have.
- The top-level payload MAY carry an optional `metrics` object for the telemetry stat rows: `cost_cumulative`/`cost_session` (`{spent,cap}`), `cache_hit_rate` (0-100), `tool_error_rate` (`{errors,total}`), `context_read_miss` (a count), `auto_continue` (`{rejected,api_error}`), and `milestone_tasks` (`{label,done,total}`). Each key is independently optional. As of this writing, this fleet has no live source for any of them - no cost/cap tracking, no cache or tool-error telemetry, no context-read-miss or auto-continue counters, and no milestone concept (a task's backlog role is not a milestone). Leave the corresponding `metrics` key out rather than fabricating a number or a fake always-zero stat; the board renders an honest "no data" card for whatever is left out. If a future source for one of these appears, wire it in then - do not pre-populate with a guess.
- Every Captain's Call item may include `filed` (the durable `.since` date, in the same format accepted for Charted Next) and `blocking` (true only when another live task lists this task id in `unresolved_blocker_ids`). The board derives urgency from filed age and blocking, and shows `-` when filed is absent. The unanswered-questions stat and table read `key` as id and `title` as question text.
