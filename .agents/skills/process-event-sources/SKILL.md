...
description: Arm or handle registered process-to-event sources. Use before arming a long-polling source or registering a condition->action watch, on any `procevent <adapter> <source-id> <sequence>` check wake, or on `process-event source stranded` / `process-event source failed to start` check wakes.
