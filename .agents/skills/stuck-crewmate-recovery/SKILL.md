...
description: Recover a stuck or missing worker after the session-start digest reports its endpoint dead or no window, a stale wake, looping pane, repeated confusion, an answered-by-brief question, an unresponsive worker, or a failed steer - or the inverse: a live worker reporting its no-mistakes pipeline dead, unreachable, or timed out.
