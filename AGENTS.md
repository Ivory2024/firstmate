...
state/               runtime records and signals; gitignored
  ...
  .quota-stop-compact-*   heartbeat quota alert-episode and task-generation dedupe markers; bin/fm-quota-stop-compact.sh owns the format
...
4. For `heartbeat:`, review the whole fleet from the structured fleet view, reconcile suspicious tasks and PR state, update the backlog, and run `bin/fm-autostart-proposal.sh` to surface quota-aware ready-work suggestions and enforce the five-hour low-quota stop/compact policy; never report an unchanged fleet as progress.
