...
Deciding a finding in-scope answers the gate directly and creates no captain-held task; only a genuine escalation holds the task per that skill's own captain-hold step, which is also what may push a Discord decision notification - a raw ask-user gate observed in a status log is never itself a captain-facing event.
