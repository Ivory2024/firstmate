...
fm_secondmate_liveness_relaunch() {  # <meta> <id> [timeout-secs]
  ...
  if [ "$FM_SM_LIVE_KILL" = 1 ]; then
    local backend target window kill_rc=0
    backend=$(fm_backend_of_meta "$meta")
    target=$(fm_backend_target_of_meta "$meta")
    if [ -z "$target" ]; then
      window=$(fm_meta_get "$meta" window)
    ...
    if [ -n "$target" ]; then
      case "$backend" in
        zellij)
          fm_backend_kill "$backend" "$target" "$(fm_meta_get "$meta" zellij_tab_id)" "fm-$id" 2>/dev/null || kill_rc=$?
          ;;
        cmux)
          fm_backend_kill "$backend" "$target" '' "fm-$id" 2>/dev/null || kill_rc=$?
          ;;
        *)
          fm_backend_kill "$backend" "$target" 2>/dev/null || kill_rc=$?
          ;;
      esac
    fi
    if [ "$kill_rc" -ne 0 ]; then
      FM_SM_LIVE_STATUS=skipped
      FM_SM_LIVE_REASON="endpoint cleanup could not be confirmed (backend=$backend)"
      FM_SM_LIVE_RC=$kill_rc
      return "$kill_rc"
    fi
  fi
  local rc=0
