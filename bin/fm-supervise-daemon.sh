...
# keyboard at the start of a message, and the pane transport sends it as text.
...
#                                   auto-discovered from $TMUX_PANE or the
#                                   firstmate:0 fallback.
#          FM_SUPERVISOR_BACKEND    supervisor pane backend (tmux; override;
#                                   otherwise auto-discovered from $TMUX_PANE,
#                                   falling back to tmux). zellij, orca, and cmux are not yet
...
# AGENTS.md section 4's harness-verification discipline. Selecting one refuses loudly at startup
...
# on any pane or its backend status-line: an OS-level macOS notification or a
...
# Post a configured notification. Best-effort: logs and returns 1 on failure.
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  # with no equivalent; the log line + durable marker above are already
...
inject_msg() {  # <message> [state]
  ...
  # dispatches through bin/fm-backend.sh so the target is checked through the
  # tmux adapter. Falls back to tmux
...
fm_super_main() {
  ...
  # Priority: FM_SUPERVISOR_BACKEND override > $TMUX_PANE > tmux fallback.
  ...
  # tmux-specific call against it (zellij, orca, and cmux have no verified
  ...
  # firstmate:0 fallback. Exporting the result into FM_SUPERVISOR_TARGET makes inject_msg
  ...
  # probe. This runs the exact same `tmux display-message -p -t "$TARGET"
