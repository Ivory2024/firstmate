...
fm_wake_status_append_self_announced() {  # <state> <status-file> <line>...
  ...
  # Record the exact append range even when the broader self-announcement
  # proof below fails because the watcher has not classified the preceding
  # worker decisions yet. Once those bytes are classified, the wake scan can
  # still recognize this home's own closes without suppressing any interleaved
  # or later worker bytes.
