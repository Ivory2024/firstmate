...
# It also applies the canonical concise, evidence-based response style to
# worker progress/status while preserving each task's exact output contract.
...
Report only evidence-backed outcomes, a verified milestone, a real blocker or approval as soon as firstmate action is needed with the recommended choice, or a next check time supported by current evidence.
On completion, include completed/total for the requested scope and the verification that passed.
