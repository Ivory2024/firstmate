...
- **A visible crew** - every crewmate works in its own tmux window, experimental Zellij tab, experimental cmux workspace, or experimental Orca terminal you can watch or type into; the first mate reconciles.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, cmux when selected or auto-detected, zellij/orca when explicitly selected); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
...
Setup guides for tmux (the default) and every other supported backend (zellij, Orca, cmux) are linked in [Documentation](#documentation) below.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    ...
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   tmux windows, zellij tabs, cmux workspaces, or Orca terminals
