...
test_backend_key_capability_matrix() {
  local backend key
  for backend in tmux zellij cmux; do
...
test_state_verified_backends_are_exactly_tmux() {
  fm_control_backend_state_verified tmux || fail "tmux has a recovery-grade classifier"
  fm_control_backend_state_verified herdr && fail "removed herdr backend has a recovery-grade classifier"
  local backend
