...
contract_enter() {
  local dir=$1
  shift
  contract_in "$dir" propose "$@" >/dev/null \
    && contract_in "$dir" confirm >/dev/null
}
...
test_return_brief_lists_landed_work_awaiting_cleanup() {
  ...
  install_runner "$dir"
  contract_enter "$dir" --words 'merge the exemption changes when green' >/dev/null 2>&1 || fail "could not confirm the away-posture record"
...
test_return_brief_keeps_refresh_history() {
  ...
  install_runner "$dir"
  contract_enter "$dir" --words 'first mandate: merge task first PR when green' >/dev/null 2>&1 || fail "could not confirm the first mandate"
...
test_missing_epoch_record_stays_required_after_disappearing() {
  ...
  contract_enter "$dir" --words 'captain words survive' >/dev/null 2>&1 || fail "could not confirm the posture record"
...
test_return_guard_refuses_while_the_record_exists() {
  ...
  install_runner "$dir"
  contract_enter "$dir" >/dev/null 2>&1 || fail "could not write the away-posture record"
  set +e
  out=$(FM_HOME="$dir/home" FM_STATE_OVERRIDE="$dir/home/state" "$dir/bin/fm-afk-return.sh" guard 2>&1)
  rc=$?
  set -e
...
SH
  ...
  : > "$dir/home/state/.fake-drain"
  out=$(PATH="$dir/fakebin:$PATH" run_return "$dir" begin) || fail "a clean fleet with a supervision gap should still clear the gate: $out"
...
test_unreadable_superseded_archive_keeps_return_gated() {
  ...
  install_runner "$dir"
  contract_enter "$dir" --words 'first mandate' >/dev/null 2>&1 || fail "could not confirm the first mandate"
  epoch=$(contract_in "$dir" field entered_epoch)
  contract_enter "$dir" --words 'replacement mandate' >/dev/null 2>&1 || fail "could not confirm the replacement mandate"
  archive=""
...
test_missing_final_archive_keeps_retained_contract_gated() {
  ...
  install_runner "$dir"
  contract_enter "$dir" --words 'durable mandate' >/dev/null 2>&1 || fail "could not confirm the mandate"
  epoch=$(contract_in "$dir" field entered_epoch)
