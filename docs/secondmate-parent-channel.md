...
The cause is structural rather than a one-off lapse: the mate can satisfy the [address rule in `docs/agent-rules-reference.md`](agent-rules-reference.md#firstmate) in local chat while missing the charter's later return-channel instruction.
