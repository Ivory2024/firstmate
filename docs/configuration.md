...
For the markdown backend, if retention has already pruned a completed item from the active backlog, teardown treats an exact completed row in this home's configured archive as already closed.
...
For a candidate with `projected_exhaustion`, include exactly one line in the brief in the form `Likely completion horizon: <positive integer> seconds`; the matching `exhaustion[]` row must provide `usableRunwaySeconds` at least that large for the candidate to remain rankable.
If the horizon or matching runway evidence is missing or invalid, the candidate stays unresolved and the resolver escalates when its limiting `spendPriority` could affect the selection.
...
The result is one of `clear` (a `profile:` line ready for `fm-spawn.sh`), `ambiguous` (confidence below the floor), `escalate` (an approval-gated rule, unverifiable rule floor, unresolved runway feasibility that could affect selection, nothing rankable, or a genuine tie), or `error` (API, network, malformed response metadata, rendering, or quota-axi failure), and every one of them exits 0.
...
By accepted design, a `clear` result does not enforce catalog/authentication or reasoning-class gates generally; the AGY `auth_required` exception above and projected-runway feasibility against an explicit horizon are enforced before ranking and dispatch output.
...
The primary Firstmate home automatically arms `bin/fm-procevent-bot-manager.sh` for the Bot Manager Notion issue database when the watcher uses the home's default state directory. An explicit `FM_STATE_OVERRIDE` suppresses this automatic registration. Its listener uses the established Notion database-query API and `NOTION_TOKEN` from `~/.claude/prompts/discord-bot.env`; its private snapshot is `state/bot-manager-autofix.json`. The first successful read establishes a baseline without backfilling old unresolved rows, then newly unresolved page IDs arrive as durable process-event results. Transport failures and HTTP 408, 429, 500, 502, 503, and 504 responses retry on the polling interval; other HTTP errors end polling with a durable `poll-error` result carrying `status: error` and the HTTP status only. Only the primary home registers this source. `bot-manager-autofix` owns judgment, deduplication of root causes, quota-aware dispatch, and review; the poller never writes to Notion or creates tasks.
...
FM_SESSION_START_QUEUED_LIMIT=20   # ready queued backlog rows in the session-start digest; in-flight, held, and blocked rows are never bounded; displayed ready order is not a priority ranking
FM_SESSION_START_DONE_LIMIT=5     # recent completed rows shown in the session-start digest; omitted completed rows are counted
