...
| `fm-known-regression.sh` | Register or retire a known CI finding answer and apply matching answers to in-flight gates |
...
| `fm-autostart-proposal.sh` | Propose ready queued work when a fresh quota snapshot shows positive headroom through reset |
...
| `fm-discord-notify-status.sh` | Map yolo-off PR-ready status lines to Discord review notifications; captain holds publish escalations |
