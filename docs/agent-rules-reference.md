# Detailed Firstmate contract

This is the on-demand reference behind the always-loaded `AGENTS.md` router.
Read only the section required by the current operation; scripts and skills remain the owners of exact mechanics.
...
`AGENTS.md` is your job description and always-loaded router; this file is its on-demand detailed contract, read section by section as the router directs.
...
Shared tracked material is `AGENTS.md`, `docs/`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and public `skills/`.
...
`configuration.md` is the single owner of the top-level operational-home layout and configuration schemas; each producing script's header and help own exact child fields and mutation mechanics.
...
```
AGENTS.md            always-loaded router (CLAUDE.md is a real @AGENTS.md pointer to it); this file is its detailed contract
...
Dispatch only on a backend that `fm-spawn` validates as spawn-capable; pass an explicit per-spawn `--backend` only under that exact task's own authority, never as later-task precedent (selection contract: [`configuration.md`](configuration.md) "Runtime backend").
...
Drive a worker's lifecycle through `bin/fm-control.sh <task-id> interrupt|exit|relaunch`, which owns the per-runtime mechanics, verifies each action, and never tears down or discards anything ([`agent-control.md`](agent-control.md)).
...
In a secondmate home, reaching the captain means appending the outcome to the parent channel your charter names; a captain-facing sentence in that home's chat has not been sent, and [`secondmate-parent-channel.md`](secondmate-parent-channel.md) owns which outcomes the home's own scripts deliver there without you.
