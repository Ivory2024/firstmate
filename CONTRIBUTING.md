...
- This repo is a template for running a firstmate orchestrator agent.
  [`AGENTS.md`](AGENTS.md) is the always-loaded router; [`docs/agent-rules-reference.md`](docs/agent-rules-reference.md) owns the detailed supervisor contract, role boundary, and bundled firstmate skill triggers it points to. `CLAUDE.md` is a real `@AGENTS.md` pointer to it, and `.claude/skills` is a symlink to `.agents/skills`.
- Only shared material is tracked: `AGENTS.md`, `docs/`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
...
Tracked changes to firstmate itself - `AGENTS.md`, `docs/`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
